...
class FocalLoss(nn.Module):
    ...
    def plot_training_history(self, history: Dict, images_path: str):
        ...
        # save figure to file
        plt.savefig(f"{images_path}/neural_network_training_history_weighted.png", dpi=300)
        plt.tight_layout()
        plt.show()

    def plot_detailed_results(self, results: Dict, images_path: str):
        ...
        # save figure to file
        plt.savefig(f"{images_path}/neural_network_detailed_results_weighted.png", dpi=300)
        plt.tight_layout()
        plt.show()
...
def main():
    ...
    graph_dir = "matej/graphs/5K_playlists/uniform"
    feature_dir = "matej/graphs/5K_playlists/uniform/features"

    train_loader, test_loader, metadata = predictor.prepare_data(
        train_labels_path=f"{graph_dir}/uniform_5k_train.csv",
        test_labels_path=f"{graph_dir}/uniform_5k_test.csv",
        features_path=f"{feature_dir}/5k_playlists_features_uniform.csv",
    )
    ...
    # Plot results
    images_path = f"{graph_dir}/images"
    predictor.plot_training_history(history, images_path)
    predictor.plot_detailed_results(results, images_path)
